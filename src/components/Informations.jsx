  ...
  return (
    <main className='presentation_page'>
      <nav className='menu_presentation'>
        <ul>
        <li className={activeCard === 'card1' ? 'active' : ''}><a href="#" onClick={() => Card('card1')}>Fondateur</a></li>
          <li className={activeCard === 'card2' ? 'active' : ''}><a href="#" onClick={() => Card('card2')}>L'entreprise</a></li>
    ...
    <h2>Présentation de l'entreprise</h2>
        ...
        <h4>Ce qui nous distingue :</h4>
        </section>
        </section>
        <aside className='description_aside'>
        <ul>
        <li className={clickedItem === 'list1' ? 'list click' : 'list inactive'} onClick={() => List('list1')}>
        {clickedItem === 'list1' ? (
          'Une approche personnalisée : Chaque projet est une nouvelle aventure, et nous nous investissons pleinement pour comprendre vos besoins et y répondre de manière innovante.'
        ) : (
          <img src="./images/file-lines-solid.svg" alt="Icone" />
        )}
      </li>
      <li className={clickedItem === 'list2' ? 'list click' : 'list inactive'} onClick={() => List('list2')}>
        {clickedItem === 'list2' ? (
          'Expertise technique : Grâce à une formation approfondie et une passion constante pour l\'apprentissage, nous maîtrisons un éventail de langages et d\'outils pour vous offrir des solutions de qualité.'
        ) : (
          <img src="./images/code-solid.svg" alt="Icone" />
        )}
      </li>

      <li className={clickedItem === 'list3' ? 'list click' : 'list inactive'} onClick={() => List('list3')}>
        {clickedItem === 'list3' ? (
          'Créativité sans limites : Nous aimons repousser les frontières du design et de la fonctionnalité pour créer des sites web qui captivent et convertissent.'
        ) : (
          <img src="./images/pen-ruler-solid.svg" alt="Icone" />
        )}
      </li>
      </ul>
